...
/*   Updated: 2018/10/22 14:42:50 by squiquem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parsing_mat(char **s, t_env *e, int *k)
{
	...
	e->mat[*k].diffuse = newcolor(ft_atoi(s[2] + 1), ft_atoi(s[3]),
			ft_atoi(s[4]));
	e->mat[*k].reflection = (double)ft_atoi(s[6]) / 100;
	e->mat[*k].specvalue = (double)ft_atoi(s[8]) / 100;
	e->mat[*k].specpower = ft_atoi(s[10]);
	e->mat[*k].n = (double)ft_atoi(s[12]) / 1000;
	(*k)++;
}
