/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   noise.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: squiquem <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/11/23 13:37:07 by squiquem          #+#    #+#             */
/*   Updated: 2018/11/23 14:18:30 by squiquem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

static int	*get_permutation(void)
{
	static int permutation[] =
	{151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53,
		194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23,
		190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117,
		35, 11, 32, 57, 177, 33, 88, 237, 149, 56, 87, 174, 20, 125, 136, 171,
		168, 68, 175, 74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83,
		111, 229, 122, 60, 211, 133, 230, 220, 105, 92, 41, 55, 46, 245, 40,
		244, 102, 143, 54, 65, 25, 63, 161, 1, 216, 80, 73, 209, 76, 132, 187,
		208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109,
		198, 173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118,
		126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28,
		42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221, 153,
		101, 155, 167, 43, 172, 9, 129, 22, 39, 253, 19, 98, 108, 110, 79, 113,
		224, 232, 178, 185, 112, 104, 218, 246, 97, 228, 251, 34, 242, 193, 238,
		210, 144, 12, 191, 179, 162, 241, 81, 51, 145, 235, 249, 14, 239, 107,
		49, 192, 214, 31, 181, 199, 106, 157, 184, 84, 204, 176, 115, 121, 50,
		45, 127, 4, 150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243,
		141, 128, 195, 78, 66, 215, 61, 156, 180};

	return (permutation);
}

static void	init_perlin(t_perlin *aux, t_vec *xyz, int *p)
{
	aux->x = (int)floor(xyz->x) % 255;
	aux->y = (int)floor(xyz->y) % 255;
	aux->z = (int)floor(xyz->z) % 255;
	aux->x = aux->x > 0 ? aux->x : 255 + aux->x;
	aux->y = aux->y > 0 ? aux->y : 255 + aux->y;
	aux->z = aux->z > 0 ? aux->z : 255 + aux->z;
	xyz->x -= floor(xyz->x);
	xyz->y -= floor(xyz->y);
	xyz->z -= floor(xyz->z);
	aux->fx = ft_fade(xyz->x);
	aux->fy = ft_fade(xyz->y);
	aux->fz = ft_fade(xyz->z);
	aux->a = p[aux->x] + aux->y;
	aux->aa = p[aux->a] + aux->z;
	aux->ab = p[aux->a + 1] + aux->z;
	aux->b = p[aux->x + 1] + aux->y;
	aux->ba = p[aux->b] + aux->z;
	aux->bb = p[aux->b + 1] + aux->z;
}

/*
**	GRAD function:
**	Converts 4 bits of hash code into 12 gradient directions
*/

double		grad(int hash, double x, double y, double z)
{
	int		h;
	double	u;
	double	v;

	h = hash % 15;
	u = h < 8 ? x : y;
	v = h < 4 ? y : 0;
	v = h == 12 || h == 14 ? x : z;
	return ((h & 1) == 0 ? u : -u) + ((h & 2) == 0 ? v : -v);
}

void		perlin(int *p, int *permutation)
{
	int			i;

	i = -1;
	while (++i < 256)
	{
		p[i] = permutation[i];
		p[256 + i] = p[i];
	}
}

double		noise(double x, double y, double z)
{
	int			p[512];
	t_vec		xyz;
	int			*permutation;
	t_perlin	aux;

	xyz = newvec(x, y, z);
	permutation = get_permutation();
	perlin(p, permutation);
	init_perlin(&aux, &xyz, p);
	return (ft_lerp(aux.fz, ft_lerp(aux.fy,
			ft_lerp(aux.fx, grad(p[aux.aa], xyz.x, xyz.y, xyz.z),
			grad(p[aux.ba], xyz.x - 1, xyz.y, xyz.z)),
			ft_lerp(aux.fx, grad(p[aux.ab], xyz.x, xyz.y - 1, xyz.z),
			grad(p[aux.bb], xyz.x - 1, xyz.y - 1, xyz.z))),
			ft_lerp(aux.fy, ft_lerp(aux.fx,
			grad(p[aux.aa + 1], xyz.x, xyz.y, xyz.z - 1),
			grad(p[aux.ba + 1], xyz.x - 1, xyz.y, xyz.z - 1)),
			ft_lerp(aux.fx,
			grad(p[aux.ab + 1], xyz.x, xyz.y - 1, xyz.z - 1),
			grad(p[aux.bb + 1],
			xyz.x - 1, xyz.y - 1, xyz.z - 1)))));
}
